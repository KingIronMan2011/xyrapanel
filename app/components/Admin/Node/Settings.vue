    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <UFormField :label="t('admin.nodes.settings.nodeName')" name="name" required>
          <UInput v-model="form.name" :placeholder="t('admin.nodes.settings.nodeNamePlaceholder')" class="w-full" />
        </UFormField>
      ...
      <UFormField :label="t('admin.nodes.settings.description')" name="description">
        <UTextarea v-model="form.description" :placeholder="t('admin.nodes.settings.descriptionPlaceholder')" :rows="3" class="w-full" />
      </UFormField>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <UFormField :label="t('admin.nodes.settings.uploadSizeLimit')" name="uploadSize">
          <UInput v-model.number="form.uploadSize" type="number" placeholder="100" min="1" max="1024" class="w-full" />
          <template #help>
    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <UFormField :label="t('admin.nodes.settings.totalMemory')" name="memory" required>
          <UInput v-model.number="form.memory" type="number" placeholder="8192" min="1" class="w-full" />
        </UFormField>

        <UFormField :label="t('admin.nodes.settings.memoryOverallocate')" name="memoryOverallocate">
          <UInput v-model.number="form.memoryOverallocate" type="number" placeholder="0" min="-1" class="w-full" />
        </UFormField>

        <UFormField :label="t('admin.nodes.settings.totalDisk')" name="disk" required>
          <UInput v-model.number="form.disk" type="number" placeholder="102400" min="1" class="w-full" />
        </UFormField>

        <UFormField :label="t('admin.nodes.settings.diskOverallocate')" name="diskOverallocate">
          <UInput v-model.number="form.diskOverallocate" type="number" placeholder="0" min="-1" class="w-full" />
        </UFormField>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 md:grid-cols-3">
        <UFormField :label="t('admin.nodes.settings.daemonPort')" name="daemonListen" required>
          <UInput v-model.number="form.daemonListen" type="number" placeholder="8080" min="1" max="65535" class="w-full" />
        </UFormField>

        <UFormField :label="t('admin.nodes.settings.sftpPort')" name="daemonSftp" required>
          <UInput v-model.number="form.daemonSftp" type="number" placeholder="2022" min="1" max="65535" class="w-full" />
        </UFormField>
